<template>
    <div class="max-w-xl mx-auto rounded shadow bg-white">
        <div class="max-w-lg mx-auto pt-16 relative">
        ...
        <div v-if="!completed && currentStep === 0">
            <div class="max-w-md mx-auto px-4 py-16 pb-10 text-center">
                <h1 class="mb-6">{{ __('Create Member') }}</h1>
                <p class="text-gray" v-text="__('statamic-memberbox::messages.member_wizard_intro')" />
            </div>
        ...
        <div v-if="!completed && currentStep === 1">
            <div class="max-w-md mx-auto px-4 py-16 text-center">
                ...
                <p class="text-gray" v-text="__('statamic-memberbox::messages.member_wizard_invitation_intro')" />
            </div>

            <!-- Send Email? -->
            <div class="max-w-md mx-auto px-4 mb-6 flex items-center justify-center">
            ...
            <!-- Copy Pasta -->
            <div class="max-w-md mx-auto px-4 pb-20 text-center" v-else>
                <p class="mb-2" v-html="__('statamic-memberbox::messages.member_wizard_invitation_share_before', { email: values.email })" />
            </div>
        </div>
        ...
        <div v-if="completed">
            <div class="max-w-md mx-auto px-4 py-16 text-center">
                <h1 class="mb-6">{{ __('Member Created') }}</h1>
                <p class="text-gray" v-html="__('statamic-memberbox::messages.member_wizard_account_created')" />
            </div>

            <!-- Copy Pasta -->
            <div class="max-w-md mx-auto px-4 pb-20 text-center" v-if="!invitation.send">
                <p class="mb-2" v-html="__('statamic-memberbox::messages.member_wizard_invitation_share', { email: values.email })" />
...
{{ __('Username') }}: {{ values.email }}
            ...
            <div class="max-w-md mx-auto px-4 pb-20 text-center" v-if="invitation.send">
                <p class="mb-2" v-html="__('statamic-memberbox::messages.member_wizard_invitation_sent', { email: values.email })" />
            </div>
        </div>
